...
class RetrofitManager private constructor() {
    ...
    fun setClientBuilder(clientBuilder: OkHttpClient.Builder): RetrofitManager {
    ...
    fun setRetrofitBuilder(retrofitBuilder: Retrofit.Builder): RetrofitManager {
    ...
    companion object {
        ...
        private val defaultInstance = RetrofitManager()
         ...
         * @return 获取新的实例
         */
        fun newInstance(): RetrofitManager {
     ...
     * 打印网络日志，仅第一次使用有效
     */
    fun printLog(print: Boolean): RetrofitManager {
        if (print) {
    ...
    fun timeout(
        ...
        callTimeout: Long? = null): RetrofitManager {
     ...
     */
    fun addInterceptors(vararg interceptors: Interceptor): RetrofitManager {
        // 需要授权的话才添加请求头，防止信息泄露
    ...
    fun ssl(factory: SSLSocketFactory, trustManager: X509TrustManager? = null): RetrofitManager {
     ...
     * 设置请求头
     */
    fun headers(headerMap: Map<String, String>): RetrofitManager {
     ...
     * 设置单个请求头，如果key已存在则替换
     */
    fun header(key: String, value: String): RetrofitManager {
     ...
     * 移除某个请求头
     */
    fun removeHeader(key: String): RetrofitManager {
     ...
     * 清除Header数据
     */
    fun clearHeader(): RetrofitManager {
        mHeaderMap.clear()
        return this
    }
